...
local joaat, toast, yield, current_time_millis, create_thread, create_tick_handler, handle_to_pointer, delete_by_handle, get_all_peds, get_all_vehicles, get_all_objects, write_int, read_int, read_long, script_global, alloc_int =
    util.joaat, util.toast, util.yield, util.current_time_millis, util.create_thread, util.create_tick_handler,
    entities.handle_to_pointer,
    entities.delete_by_handle, entities.get_all_peds_as_handles, entities.get_all_vehicles_as_handles,
    entities.get_all_objects_as_handles, memory.write_int, memory.read_int, memory.read_long, memory.script_global,
    memory.alloc_int
local my_root, player_root, ref_by_path, ref_by_rel_path, trigger_command = menu.my_root, menu.player_root,
    menu.ref_by_path, menu.ref_by_rel_path, menu.trigger_command
...
local function set_network_attributes(entity, exists_all_machines, can_migrate, can_be_reassigned,
                                      disable_proximity_migration)
...
local function create_entity(create_func, modelhash, pos, additional_args, exists_all_machines, can_migrate,
                             can_be_reassigned, disable_proximity_migration)
    local pos = pos or v3()
    local status = request_model(modelhash)
    if status then
        local entity = create_func(modelhash, pos, table.unpack(additional_args))
        if entity ~= 0 then
            set_network_attributes(entity, exists_all_machines, can_migrate, can_be_reassigned,
                disable_proximity_migration)
...
function create_ambient_pickup(pickuphash, pos, flags, value, modelhash, p7, p8, exists_all_machines, can_migrate,
                               can_be_reassigned, disable_proximity_migration) -- 6
    ...
    return create_entity(create_func, modelhash, pos, { pickuphash, flags, value, modelhash, p7, p8 },
        exists_all_machines, can_migrate, can_be_reassigned, disable_proximity_migration)
end

function create_object_no_offset(modelhash, pos, isnetworked, scripthostobj, dynamic, exists_all_machines, can_migrate,
                                 can_be_reassigned, disable_proximity_migration)
    ...
    return create_entity(create_func, modelhash, pos, { isnetworked, scripthostobj, dynamic }, exists_all_machines,
        can_migrate, can_be_reassigned, disable_proximity_migration)
end

function create_ped_inside_vehicle(vehicle, pedtype, modelhash, seat, isnetworked, scripthostped, exists_all_machines,
                                   can_migrate, can_be_reassigned, disable_proximity_migration) -- 6
    ...
    return create_entity(create_func, modelhash, v3(), { vehicle, pedtype, seat, isnetworked, scripthostped },
        exists_all_machines, can_migrate, can_be_reassigned, disable_proximity_migration)
end

function create_ped(pedtype, modelhash, pos, heading, isnetworked, scripthostped, exists_all_machines, can_migrate,
                    can_be_reassigned, disable_proximity_migration) -- 6
    ...
    return create_entity(create_func, modelhash, pos, { pedtype, heading, isnetworked, scripthostped },
        exists_all_machines, can_migrate, can_be_reassigned, disable_proximity_migration)
end

function create_object(modelhash, pos, isnetworked, scripthostobj, dynamic, exists_all_machines, can_migrate,
                       can_be_reassigned, disable_proximity_migration) -- 7
    local create_func = function(modelhash, pos, isnetworked, scripthostobj, dynamic)
...
function create_vehicle(modelhash, pos, heading, isnetworked, scripthostveh, p7, exists_all_machines, can_migrate,
                        can_be_reassigned, disable_proximity_migration) -- 7
    ...
    return create_entity(create_func, modelhash, pos, { heading, isnetworked, scripthostveh, p7 }, exists_all_machines,
        can_migrate, can_be_reassigned, disable_proximity_migration)
end

function set_entity_as_networked(Entity, timeout)
...
function constantize_network_id(Entity)
...
function request_control_of_id(net_id, time_to_wait)
...
function request_control_of_entity(entity, time_to_wait)
...
function Animation.play(ped, EmoteName)
    ...
    if ChosenDict == "MaleScenario" or ChosenDict == "Scenario" then
        ...
        elseif ChosenDict == "ScenarioObject" then
            TASK.TASK_START_SCENARIO_AT_POSITION(ped, ChosenAnimation,
                ENTITY.GET_ENTITY_COORDS(ped) - vector3(0.0, 0.0, 0.5), ENTITY.GET_ENTITY_HEADING(ped), 0, true, false)
        else
            TASK.TASK_START_SCENARIO_IN_PLACE(ped, ChosenAnimation, 0, true)
        end
    ...
    TASK.TASK_PLAY_ANIM(ped, ChosenDict, ChosenAnimation, 2.0, 2.0, AnimationDuration, MovementType, 0.0, false, false,
        false)
...
function Animation.cancel(ped)
    TASK.CLEAR_PED_TASKS(ped)
    AnimationProp.destroy_all(ped)
end

function AnimationProp.add(ped, propmodel, bone, off1, off2, off3, rot1, rot2, rot3)
    local prop = create_object(joaat(propmodel), ENTITY.GET_ENTITY_COORDS(ped), true, false, false, true, false, false,
        true)
    ENTITY.SET_ENTITY_INVINCIBLE(prop, true)
    ENTITY.ATTACH_ENTITY_TO_ENTITY(prop, ped, PED.GET_PED_BONE_INDEX(ped, bone), off1 or 0.0, off2 or 0.0, off3 or 0.0,
        rot1 or 0.0, rot2 or 0.0, rot3 or 0.0, false, false, false, false, 1, true)
    ENTITY.SET_ENTITY_COMPLETELY_DISABLE_COLLISION(prop, false, true)
    table.insert(Emote_Props, prop)
end

function AnimationProp.destroy_all(ped)
